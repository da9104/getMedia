...
var iTunesGenre = async () => {
    try {
            ...
            let text = "";
            for (let i = 0; i < 10; i++) {     
            text += `<span id="genre" style="margin: 0 auto; border-radius: 25px; ">${res.data.results[i].primaryGenreName}</span>`
        }
        return text;
    }
    catch (e) {
...
var getGenre = async () => {
    ...
    setTimeout(() => {
    document.querySelector('#genre').innerHTML = `${genres}.append("<button id='listButtonAdd'> Button</button>")`;
    },3).then(
